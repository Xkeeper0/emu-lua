...
function getready()
	...
	while not ready do
		...
		if timer == 90 then
			for i = 0, 70 do
				balls[z]	= {x = 203, y = 11, xs = math.random(-90, 90) / 100, ys = math.random(-90, 90) / 100, life = 60, c = 0};
				z			= z + 1;
			end;
		end;
...
--		text(8, 220, string.format("Looking for %s...", secret[secreti]));
		if inputthis[secret[secreti]] and not inputlast[secret[secreti]] then
			tempx	= math.random(50, 200);
			tempy	= math.random(50, 200);
			if secreti == 10 then
				ultramode	= true;
				player['life']	= 1;
				player['lifem']	= 0.5;
			...
			secreti	= math.fmod(secreti, 10) + 1;
			for i = 0, 20 do
				balls[z]	= {x = tempx, y = tempy, xs = math.random(-200, 200) / 100, ys = math.random(-200, 200) / 100, life = math.random(60, 120), c = 0};
				z			= z + 1;
			end;

		elseif inputthis['A'] and not inputlast['A'] then
			ready	= true;
		end;
		FCEU.frameadvance();
	end;
