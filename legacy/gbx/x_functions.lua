...
--[[
	...
	v8 -----------
	- VBA-rr broke something fantastically, adding workaround for colors (gui.fixcolor)

]]



function gui.fixcolor(c)
	if not c then 
		return c;
	end;
	r	= bit.band(bit.rshift(c, 24), 0xff);
	g	= bit.band(bit.rshift(c, 16), 0xff);
	b	= bit.band(bit.rshift(c,  8), 0xff);
	a	= bit.band(           c     , 0xff);
...
function line(x1,y1,x2,y2,color)
	if (x1 >= 0 and x1 <= 255 and x2 >= 0 and x2 <= 255 and y1 >= 0 and y1 <= 244 and y2 >= 0 and y2 <= 244) then
		local success = pcall(function() gui.drawline(x1,y1,x2,y2,gui.fixcolor(color)) end);
		if not success then 
...
function text(x, y, str, c1, c2)

	if str == nil then
		str = "nil";
	end;

	gui.text(x, y, str, gui.fixcolor(c1), gui.fixcolor(c2));
...
function pixel(x,y,color)
	if (x >= 0 and x <= 255 and y >= 0 and y <= 240) then
		gui.drawpixel(x,y,gui.fixcolor(color));
	end;
end;
...
--[[	local success = pcall(function() gui.drawbox(x1,y1,x2,y2,color); end);
		...
		gui.drawbox(x1,y1,x2,y2,0x00000000, gui.fixcolor(color));
	end;
end;
...
--	gui.fillbox(x1,y1,x2,y2,color);
	gui.box(x1,y1,x2,y2, gui.fixcolor(color));
end;
