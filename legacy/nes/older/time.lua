...
--	a		= a + 100;
	...
	local xss	= math.random(-2, 2);
	local yss	= math.random(-7, -2);
	
	for i = 1,a do
		local temp	= math.random(0, 200000) / 100000 * math.pi * 2;
		local temp2	= math.random(0 + pow * 45, 250) / 100;
		local xs			= math.sin(temp) * temp2 * p + xss;
		local ys			= math.cos(temp) * temp2 * p + yss;
...
--	line( 86, 174, 162, 174, "blue");
	...
	if hide <= 70 then
		...
		for i = 8, 20 do
			line( 80, 168 - i, 160, 168 - i, "#000000");
			line( 80, 167 + i, 160, 167 + i, "#000000");
		end;
