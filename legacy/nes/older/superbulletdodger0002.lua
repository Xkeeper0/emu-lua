...
--		box(b2x1, b2y1, b2x2, b2y2, "#ffffff");
		for i = 0, 10 do
			balls[z]	= {x = player['x'], y = player['y'], xs = math.random(-100, 100) / 100, ys = math.random(-100, 100) / 100, life = math.random(15, 30), c = 0};
			z			= z + 1;
		end;
		return true;
	else
...
--	if math.fmod(timer, math.ceil(ballrate)) == 0 then
	if balltimer < 0 then
		balls[z]	= {x = math.random(0, 0xFF), y = 0, xs = math.random(-50, 50) / 100, ys = math.random(100, 400) / 100, life = 240, c = 2, power = 1};
		z			= z + 1;
		ballcolor	= "#ffffff";
...
--	lifebar(30, 231, 200, 4, cpu - 1, 200, "#ff8800", "#000000");
	...
	text(  2, 222, string.format("%6dpts", math.min(999999, score)));
	text(  5, 230, string.format("%3d/%3d", player['life'], player['lifem']));
...
while true do
	...
	if timer >= 110 then
		...
		tpos	= math.max(120 - atimer, 0);
		atpos	= math.floor(math.pow(tpos / 5, 2));
		text(68, 80 + atpos, string.format("Final Score%8dpts", score));
	end;
	if timer >= 120 then
		atimer	= timer - 120;
		tpos	= math.max(120 - atimer, 0);
		atpos	= math.floor(math.pow(tpos / 5, 2));
		text(69, 89 + atpos, string.format("Final Time  %4d'%02d\"%02d", math.floor(totaltime / 3600), math.floor(math.fmod(totaltime, 3600) / 60), math.fmod(totaltime, 60)));
	end;
	if timer >= 60 then
		atimer	= timer - 60;
		tpos	= math.max(120 - atimer, 0);
