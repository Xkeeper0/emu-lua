...
--				text(e2x1 - 5, e2y1 - 13, string.format("%02X", memory.readbyte(0x001E + i)));
				...
				enemyxpos	= memory.readbyte(0x006D + i) * 0x0100 + memory.readbyte(0x0086 + i);
				enemyypos	= memory.readbyte(0x00B5 + i) * 0x100 + memory.readbyte(0x00CE + i);
				enemyxposa	= enemyxpos - smbdata['screenpos'];
				enemyyposa	= enemyypos - 0x100;
--				text(8, 10 + 8 * i, string.format("%04X %04X %04X %04X", enemyxpos, enemyypos, enemyxposa, enemyyposa));
